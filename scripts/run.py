...
try:
    ...
    # Maybe one want's to run the code without installing it.
...
if __name__ == "__main__":
    ...
    if args.train_data is not None:
        options = {x.split("=")[0]:
                   ubase.heuristic_conversion(x.split("=")[1])
        ...
        X, y = ubase.get_data(args.train_data, solver.dtype)
    ...
    if args.test_data is not None:
        try:
            ...
            y_hat, msg = ubase.time_it(lambda: solver.predict(X), "Test")
            log.info(msg)
    ...
    if args.train_data is not None and args.model_dir is not None:
        ...
        _, msg = ubase.time_it(f, "Serializing")
        log.info(msg)
    pass
